...
- **L2 已修**（7e98070）：队列执行体不再覆写用户输入框草稿——SendAsync 增 textOverride 参数，队列直传文本，仅消费输入框时才清空。
- **L1 已修**：附件历史驱逐——HistoryMapper 仅保留最近 2 个用户轮的完整附件注入正文，更早附件轮降级为「元信息存根 + 原文」；新增 chat_messages.UserText 锚点列（存量库幂等补列），DB/UI 保真不受影响。附件上下文总量钉死在 ≤2×120K 字符硬上界。规格见 docs/ATTACHMENT_EVICTION_SPEC.md。
- **L4 已修**：注入正文与用户文本的分隔符计入注入预算（预算计量语义按 BuildInjection 既有口径，见规格 I4）。
- **独立评审（builder≠verifier）闭环**：MED-1 steer 插话不计保留窗口（防当前轮附件被顶出）；MED-2 hostile 元数据逐项防御不抛异常；MED-3 编辑/重跑/分叉运行优先取 UserText 锚点（防注入正文以不可驱逐形态复活）；LOW-3 会话自动标题取原文锚点。
- 验证：15 个新用例 + 1 个扩展用例全过 + 全量回归（见提交）。
