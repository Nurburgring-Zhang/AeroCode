...
public partial class AIAssistantViewModel : ObservableObject
{
    ...
    // ── UIR-5：指令排队队列（可复用 CommandQueueEngine：自动执行 + 排序/插队/编辑/删除/折叠）──
    /// <summary>指令队列引擎（共享组件；执行体在构造函数注入为本页 SendAsync）。</summary>
    ...
    // ============== UIR-5：指令排队队列（编排委托给可复用 CommandQueueEngine） ==============

    /// <summary>把输入框指令加入队列（不立即执行）。空闲时引擎自动开始执行。</summary>
    [RelayCommand]
