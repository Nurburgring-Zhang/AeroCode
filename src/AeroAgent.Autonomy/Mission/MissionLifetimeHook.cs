...
// finally 触发 MissionStopped(missionId)。Android 侧 MainActivity 订阅后把设置
...
public static class MissionLifetimeHook
{
    ...
    /// <summary>mission 结束（成功/失败/取消统一；参数 = missionId，R4 δ-4：
    /// 停止侧据此只解除该 mission 的保活，不误清其他在保 mission）；订阅方必须不抛。</summary>
    public static Action<string>? MissionStopped;
}
